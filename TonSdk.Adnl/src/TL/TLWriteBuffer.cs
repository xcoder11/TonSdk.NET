...
public class TLWriteBuffer
{
    ...
    private void EnsureSize(int needBytes)
    {
        ...
        {
            var newLength = (int)_stream.Length * 2;
    ...
    public void WriteInt256(BigInteger val)
    {
        ...
        buffer.Clear();
        if (!val.TryWriteBytes(buffer, out var bytesWritten)) throw new Exception("Invalid int256 length");
    ...
    public void WriteVector<T>(Action<T, TLWriteBuffer> codec, T[] data)
    {
        ...
        foreach (var d in data) codec(d, this);
    }
